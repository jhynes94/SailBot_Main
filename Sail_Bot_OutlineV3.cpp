...
void TackingFunction(int DesiredHeading);

void TrimmingFunction(int DesiredHeading);

int TackOrTrim(double Boat_GPS_X, double Boat_GPS_Y,double Des_GPS_X,double Des_GPS_Y, int Wind_Dir, int &Heading);

int main(void)
{
     double Boat_GPS_X, Boat_GPS_Y, Des_GPS_X, Des_GPS_Y;
     int Wind_Dir;
     int DesiredHeading;  //This is solved and fed back through Tack or Trim function
     
     cout << "The following program emulates the functions used for the Sail Bot V3\n\n"; //Add a Last edited Function here
              //Get Values
     ...
     if (TackOrTrim(Boat_GPS_X, Boat_GPS_Y, Des_GPS_X, Des_GPS_Y, Wind_Dir, DesiredHeading) == 0)
       TackingFunction(DesiredHeading);
     else
       TrimmingFunction(DesiredHeading);      
...
void TackingFunction(int DesiredHeading){
     ...
     */
     ...
     //For now the default tack will be 45 degrees clockwise from the wind
     //Thus the 45 degrees is added to the desired heading
     DesiredHeading = DesiredHeading + 45;     
     
     if ((DesiredHeading - Compass_Heading < -180) || (DesiredHeading - Compass_Heading > 0)){
...
void TrimmingFunction(int DesiredHeading){
     ...
     //-----------Turn Rudders to make boat goto heading--------     
     if ((DesiredHeading - Compass_Heading < -180) || (DesiredHeading - Compass_Heading > 0)){
            cout << "\nBoat is Turning Clockwise\n";
            //Turn Clockwise
            }
